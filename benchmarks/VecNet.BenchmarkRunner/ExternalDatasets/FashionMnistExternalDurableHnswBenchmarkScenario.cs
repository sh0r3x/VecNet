...
public static class FashionMnistExternalDurableHnswBenchmarkScenario
{
    private const string TaskId = "VEC-109";
    private const string SchemaName = "VecNet.ExternalDurableHnswBenchmarkReport";
    ...
    public static ExternalDurableHnswBenchmarkReport Run(
        ...
        IReadOnlyList<string> commandArguments)
    {
        ...
        var sourceSearchRuns = new SearchRunInfo[options.Runs];
        ...
        for (int runIndex = 0; runIndex < options.Runs; runIndex++)
        {
            ...
            SingleRunMeasurement sourceSearch = MeasureSearch(options, dataset, build.Index, captureResults);
            sourceSearchRuns[runIndex] = sourceSearch.Summary with { RunNumber = runIndex + 1 };

            WarmupSearch(options, dataset, open.Index);
            SingleRunMeasurement openedSearch = MeasureSearch(options, dataset, open.Index, captureResults);
            ...
            if (captureResults)
            {
                ...
                finalSourceResults = sourceSearch.Results;
        ...
        {
            throw new InvalidOperationException("At least one external durable HNSW run is required.");
        }
        ...
        AggregateTimingInfo sourceSearchAggregate = AggregateSearchRuns(sourceSearchRuns, options.QueryCount);
        AggregateTimingInfo openedSearchAggregate = AggregateSearchRuns(openedSearchRuns, options.QueryCount);

        return new ExternalDurableHnswBenchmarkReport(
            SchemaName,
            SchemaVersion,
            ...
            new CommandInfo(FashionMnistExternalDurableHnswBenchmarkOptions.ScenarioName, commandArguments.ToArray()),
            new EnvironmentInfo(
            ...
            new ScenarioInfo(
                ...
                options.QueryCount,
                1,
                "cache checks, manifest checksum validation, matrix load, truth load, warmup queries, validation, parity comparison, output-byte scans and report writing are excluded from measured build/save/open/search durations"),
            new IndexInfo(
                "ExternalDurableInternalHnswEvaluation",
                nameof(HnswIndex),
                options.Metric.ToString(),
                dataset.Dimension,
                dataset.BaseCount,
                "internal/evaluation-only HnswIndex built from admitted converted base matrix, saved to a private snapshot directory and opened read-only; no public API, hostile-open validation, comparison artifact, baseline policy or regression gate"),
            ...
            new DurableHnswWorkloadInfo(
                ...
                "admitted base matrix row order, external ids 0..baseCount-1",
                "source HNSW built from admitted Fashion-MNIST vectors, saved to a fresh ignored per-run snapshot directory and opened read-only",
                "hnsw"),
            new ExternalDurableHnswOperationsInfo(
                new DurableHnswOperationInfo(
                    "build",
                    "internal HnswIndex construction and Add calls for admitted Fashion-MNIST base vectors",
                    buildRuns,
                    buildAggregate),
                new DurableHnswOperationInfo(
                    "save",
                ...
                new DurableHnswOpenedSearchOperationInfo(
                    "sourceSearch",
                    "internal source HnswIndex.Search(query, results, workspace)",
                    sourceSearchRuns,
            ...
            new ExternalDurableHnswMeasurementInfo(
                CreateOperationMeasurement(
                    "build",
                    "internal HnswIndex construction and Add calls for admitted Fashion-MNIST base vectors",
                    "cache checks, manifest checksum validation, matrix load, truth load, save, open, warmup, source/opened search, validation, parity comparison, output-byte scans and report writing",
                    buildRuns,
                    new MeasurementStatusInfo(
                        "measured",
                        buildAllocatedBytes.Average().ToString(CultureInfo.InvariantCulture),
                        "bytesPerBuild",
                        "Measured with GC.GetAllocatedBytesForCurrentThread around internal HnswIndex construction and Add calls only; external cache/truth loading is excluded.")),
                CreateOperationMeasurement(
                    "save",
                    "internal HnswIndex.Save(directoryPath)",
                    "cache checks, matrix/truth loading, HNSW build, target directory name generation, validation searches, output-byte scans, OpenReadOnly, source/opened search, cleanup outside the save call and report writing",
                    saveRuns,
                    NotMeasured("bytesPerSaveCall", "Managed allocation for HnswIndex.Save(directoryPath) is not measured in VEC-109.")),
                CreateOperationMeasurement(
                    "open",
                    "internal HnswIndex.OpenReadOnly(directoryPath)",
                    "cache checks, matrix/truth loading, HNSW build, save, durable output byte scans, exact truth loading, source/opened search result comparison, warmup and report writing",
                    openRuns,
                    NotMeasured("bytesPerOpenCall", "Managed allocation for HnswIndex.OpenReadOnly(directoryPath) is not measured in VEC-109.")),
                CreateSearchMeasurement(options, sourceSearchRuns, "source", "internal source HnswIndex.Search(query, results, workspace)"),
                CreateSearchMeasurement(options, openedSearchRuns, "opened", "internal opened HnswIndex.Search(query, results, workspace)"),
                ...
                new WarmupInfo(
                    ...
                    options.WarmupQueries > 0
                        ? "Warmup queries executed against both source and opened indexes before their measured search operations and excluded from measured operation timings."
                        : "No warmup queries were requested."),
                "External cache checks, manifest checksum validation, matrix load, truth load, warmup queries, validation, source/opened parity comparison, output-byte scans and report writing are excluded from the relevant measured operations."),
            new DurableHnswOutputsInfo(
                snapshotOutput,
                NotMeasured("bytes", "Temporary disk and failed-save remnants are not measured in VEC-109."),
                NotMeasured("bytes", "Peak temporary disk usage is not sampled in VEC-109.")),
            new DurableHnswMetricsInfo(
                CreateMetricsInfo(sourceComparison, sourceIntegrity, sourceExtraResultCount),
                CreateMetricsInfo(openedComparison, openedIntegrity, openedExtraResultCount),
                ...
                parity.AllResultsMatched
                    ? "Source/opened parity preserved result sets for the same admitted Fashion-MNIST query subset, so recall and distance-integrity are expected to match."
                    : "Source/opened result parity failed; recall-equivalence fields are retained as private smoke diagnostics."),
            new ExternalDurableHnswValidationInfo(
                validationPassed ? "passed" : "failed",
                "external-durable-hnsw-smoke",
            ...
            CreateMemoryEstimates(options, dataset, finalSourceIndex, snapshotOutput.TotalBytes),
            CreateEligibility(),
            [
                "Private external Fashion-MNIST durable-HNSW smoke evidence only; not a public benchmark claim.",
                "This report loads an already admitted Fashion-MNIST cache and existing exact truth artifact; it does not download data or generate truth.",
                "Build, save, open, source-search and opened-search timings are separate operation blocks.",
                "Source and opened searches use caller-owned SearchResult[] and HnswSearchWorkspace.",
                "Output bytes are private local file facts scanned outside save/open duration, not public persisted-size claims.",
                "Actual resident/process/GC/private/peak memory is explicitly not measured.",
                "No public HNSW API/profile admission, baseline candidate, comparison artifact, regression gate, hostile durable-open validation, concurrency evidence or public documentation is included."
            ]);
    }

    public static void Write(ExternalDurableHnswBenchmarkReport report, string outputPath) =>
        ReportWriter.WriteJson(report, outputPath);

    private static FashionMnistExternalHnswBenchmarkScenario.LoadedExternalDataset LoadDataset(FashionMnistExternalDurableHnswBenchmarkOptions options) =>
    ...
    private static SingleRunMeasurement MeasureSearch(
        FashionMnistExternalDurableHnswBenchmarkOptions options,
        FashionMnistExternalHnswBenchmarkScenario.LoadedExternalDataset dataset,
        HnswIndex index,
        bool captureResults)
    {
        ...
        for (int queryRow = 0; queryRow < options.QueryCount; queryRow++)
        {
            long allocationStart = GC.GetAllocatedBytesForCurrentThread();
            long start = Stopwatch.GetTimestamp();
            int written = index.Search(dataset.GetQueryVector(queryRow), results, workspace);
            long elapsed = Stopwatch.GetTimestamp() - start;
            long allocatedBytes = GC.GetAllocatedBytesForCurrentThread() - allocationStart;
    ...
    private static DurableHnswParityInfo CompareSourceOpenedParity(SearchResult[][] sourceResults, SearchResult[][] openedResults)
    {
        ...
        return new DurableHnswParityInfo(
            ...
            distanceMismatchCount,
            passed,
            "Source/opened parity requires source and opened HNSW results to match written count, IDs, result order and exact returned distances for every validation query.");
    }
    ...
    private static DurableHnswSnapshotOutputInfo InspectSnapshotOutput(string snapshotDirectory, int vectorCount)
    {
        ...
        return new DurableHnswSnapshotOutputInfo(
            "written",
            "caller-selected snapshot root with fresh per-run external durable HNSW snapshot directory",
            snapshotDirectory,
            FileCount: 5,
            totalBytes,
            manifestBytes,
    ...
    private static DurableHnswOperationMeasurementInfo CreateOperationMeasurement(
        ...
        new(
            new LatencyMeasurementInfo(
                ...
                "Mean/min/max are private local descriptive metadata across independently rebuilt equivalent external durable-HNSW runs, not BenchmarkDotNet statistics.",
    ...
    private static DurableHnswSearchMeasurementInfo CreateSearchMeasurement(
        FashionMnistExternalDurableHnswBenchmarkOptions options,
        SearchRunInfo[] runs,
        string searchKind,
        string timedOperation) =>
        new(
            new LatencyMeasurementInfo(
                "measured",
                "milliseconds",
                string.Create(CultureInfo.InvariantCulture, $"perMeasured{CultureInfo.InvariantCulture.TextInfo.ToTitleCase(searchKind)}Query"),
                timedOperation,
                "cache checks, manifest checksum validation, matrix/truth loading, build, save, open, warmup queries, final-run result comparison, source/opened parity validation, returned-result integrity validation, output-byte scans and report writing",
                ...
                "Top-level search aggregate percentile fields are arithmetic means across per-run percentile values, not BenchmarkDotNet statistics.",
            ...
            new MeasurementStatusInfo(
                ...
                "bytesPerQuery",
                string.Create(CultureInfo.InvariantCulture, $"Measured with GC.GetAllocatedBytesForCurrentThread around each {timedOperation} call using caller-owned SearchResult[] and HnswSearchWorkspace; setup, build, save, open, warmup, validation and report writing are excluded.")),
            CreateRepeatedRunInfo(runs.Length, string.Create(CultureInfo.InvariantCulture, $"{searchKind}-search operation")),
            CreateSearchRunToRunNoise(runs, options.QueryCount, searchKind));
    ...
    private static RunToRunMetricNoiseInfo CreateOperationRunToRunNoise(DurableHnswOperationRunInfo[] runs, string operationName)
    {
        ...
        return new RunToRunMetricNoiseInfo(
            ...
            "Computed across measured external durable-HNSW operation runs using the documented private descriptive-statistics formula.");
    }

    private static RunToRunNoiseInfo CreateSearchRunToRunNoise(SearchRunInfo[] runs, int measuredQueryCountPerRun, string searchKind)
    {
        bool measured = runs.Length > 1;
        string status = measured ? "measured" : "notMeasured";
        string reason = measured
            ? $"Multiple measured {searchKind}-search runs executed; simple descriptive run-to-run statistics are recorded for private local external durable-HNSW noise inspection."
            : $"Only one measured {searchKind}-search run executed, so run-to-run noise is unavailable and cannot be measured.";
        string unavailableReason = $"Only one measured {searchKind}-search run exists; this field does not establish run-to-run variation.";

        return new RunToRunNoiseInfo(
            status,
            runs.Length,
            measured,
            string.Create(CultureInfo.InvariantCulture, $"Across measured {searchKind} HNSW search runs with {measuredQueryCountPerRun} measured queries per run; warmup, build, save, open, validation and report writing are excluded."),
    ...
    private static DurableHnswMemoryEstimateInfo CreateMemoryEstimates(
        FashionMnistExternalDurableHnswBenchmarkOptions options,
        FashionMnistExternalHnswBenchmarkScenario.LoadedExternalDataset dataset,
        HnswIndex index,
        long durableOutputBytes)
    {
        ...
        for (int layer = 0; layer < layerCount; layer++)
        {
            ...
            graphNeighborBytes = checked(graphNeighborBytes + (long)dataset.BaseCount * stride * sizeof(int));
            graphCountBytes = checked(graphCountBytes + (long)dataset.BaseCount * sizeof(int));
        }
        ...
        return new DurableHnswMemoryEstimateInfo(
            ...
            checked((long)dataset.BaseCount * dataset.Dimension * sizeof(float)),
            checked((long)dataset.BaseCount * sizeof(ulong)),
            checked((long)dataset.BaseCount * sizeof(int)),
            graphCountBytes,
            graphNeighborBytes,
            EstimateWorkspaceBytes(dataset.BaseCount, options.EfSearch),
            durableOutputBytes,
    ...
    private static DurableHnswEvidenceInfo CreateEvidence() =>
        new(
            ...
            ComparisonArtifactEligible: false,
            RegressionGateEligible: false,
            "Private external durable-HNSW smoke output is not reviewed public evidence and has no public reporting policy.",
            "One local external durable report does not establish hostile durable-open validation, actual memory, peak memory, concurrency, Linux, package or public API readiness.",
            "No external durable-HNSW baseline-candidate policy is accepted.",
            "No external durable-HNSW comparison artifact schema or compatibility policy is accepted.",
            "No external durable-HNSW threshold, hard gate or regression decision policy is accepted.",
            [
                "Private external Fashion-MNIST durable-HNSW smoke evidence only.",
                "Build, save, open, source-search and opened-search operation boundaries are separated.",
    ...
    private static void ValidateOptions(FashionMnistExternalDurableHnswBenchmarkOptions options)
    {
        ...
        {
            throw new ArgumentException("ef-search must be greater than or equal to top-k and no more than 4096.", nameof(options));
        }
    }
    ...
    private static string CreateReportId(string? commit, FashionMnistExternalDurableHnswBenchmarkOptions options)
    {
        string commitPart = string.IsNullOrWhiteSpace(commit) ? "unknown" : commit[..Math.Min(12, commit.Length)];
        return string.Create(
            CultureInfo.InvariantCulture,
            $"{FashionMnistExternalDurableHnswBenchmarkOptions.ScenarioName}-{commitPart}-{options.QueryCount}q-{options.TopK}k-{options.Runs}r-{options.WarmupQueries}w-m{options.M}-efc{options.EfConstruction}-efs{options.EfSearch}-{options.HnswSeed:X16}");
    }
