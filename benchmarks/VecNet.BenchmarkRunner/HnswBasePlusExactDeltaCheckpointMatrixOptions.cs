...
public sealed record HnswBasePlusExactDeltaCheckpointMatrixOptions(
    string PresetName,
    int BaseVectorCount,
    int QueryCount,
    int Runs,
    ...
    VectorMetric Metric = VectorMetric.SquaredEuclidean)
{
    ...
    public static string NormalizePresetName(string presetName)
    {
        ...
        throw new ArgumentException($"Unsupported HNSW base-plus-exact-delta checkpoint matrix preset '{presetName}'.");
    }
}
