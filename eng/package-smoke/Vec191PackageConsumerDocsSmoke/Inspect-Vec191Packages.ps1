...
function Expand-Package {
    ...
    $expanded = Join-Path ([System.IO.Path]::GetTempPath()) ("vec191-package-inspect-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    if ($null -eq $vecNetDependency -or -not (Test-AcceptedDependencyVersion -Actual $vecNetDependency.Version -Expected $core.Version)) {
        throw "Adapter package does not depend on accepted VecNet version $($core.Version)."
    }
    ...
    Write-Host "VEC191_CORE_PACKAGE id=$($core.Id) version=$($core.Version)"
    Write-Host "VEC191_CORE_PACKAGE_FILES"
    $core.Files | ForEach-Object { Write-Host "  $_" }
    Write-Host "VEC191_ADAPTER_PACKAGE id=$($adapter.Id) version=$($adapter.Version)"
    Write-Host "VEC191_ADAPTER_PACKAGE_FILES"
    $adapter.Files | ForEach-Object { Write-Host "  $_" }
    Write-Host "VEC191_ADAPTER_DEPENDENCIES"
