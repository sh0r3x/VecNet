...
$consumerSource = Join-Path $repoRoot "eng/package-smoke/Vec191DocsConsumer"
...
function Assert-PackageReferenceRestore {
    $projectPath = Join-Path $consumerRoot "Vec191DocsConsumer.csproj"
    ...
    foreach ($requiredPackageId in @("VecNet", "VecNet.Integration.VectorData")) {
        $library = $assets.libraries.PSObject.Properties |
            Where-Object { $_.Name -like "$requiredPackageId/*" } |
            Select-Object -First 1
        if ($null -eq $library -or $library.Value.type -ne "package") {
            throw "$requiredPackageId was not resolved as a NuGet package."
        }
    }
