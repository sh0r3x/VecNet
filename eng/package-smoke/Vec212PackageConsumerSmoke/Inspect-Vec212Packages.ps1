...
function Expand-Package {
    ...
    $expanded = Join-Path ([System.IO.Path]::GetTempPath()) ("vec212-package-inspect-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    Write-Host "VEC212_CORE_PACKAGE id=$($core.Id) version=$($core.Version)"
    Write-Host "VEC212_CORE_PACKAGE_FILES"
    $core.Files | ForEach-Object { Write-Host "  $_" }
    Write-Host "VEC212_ADAPTER_PACKAGE id=$($adapter.Id) version=$($adapter.Version)"
    Write-Host "VEC212_ADAPTER_PACKAGE_FILES"
    $adapter.Files | ForEach-Object { Write-Host "  $_" }
    Write-Host "VEC212_ADAPTER_DEPENDENCIES"
